...
from custom_components.elta_courier.api import ELTACourierApiError
...
from custom_components.elta_courier.coordinator import (
    ELTACourierCoordinator,
...
from .payloads import ACTIVE_CODE, STATUS_IN_TRANSIT, STATUS_REGISTERED, event
...
async def test_update_interval_resumes_once_a_parcel_is_added(hass):
    ...
    # In transit, not yet out for delivery — mid tier, not hot.
    client.async_get_parcel.return_value = {
        "tracking_code": ACTIVE_CODE,
        "echoed_tracking_code": ACTIVE_CODE,
        "events": [
...
async def test_429_raises_update_failed_with_retry_after(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.side_effect = ELTACourierApiError(
