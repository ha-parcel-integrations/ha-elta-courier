"""Tests for ELTA Courier setup and unload."""
...
async def test_setup_and_unload(hass):
    ...
    with patch(
        "custom_components.elta_courier.api.ELTACourierApiClient.async_get_parcel",
    ...
    incoming = hass.states.get("sensor.elta_courier_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.elta_courier.api.ELTACourierApiClient.async_get_parcel",
        new=AsyncMock(side_effect=ELTACourierApiError("ELTA Courier unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    ...
    with patch("custom_components.elta_courier.api.ELTACourierApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
