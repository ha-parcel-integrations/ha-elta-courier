"""Tests for the ELTA Courier coordinator: fetching, caching and events.
...
from custom_components.elta_courier import parcels
...
from custom_components.elta_courier.coordinator import ELTACourierCoordinator
...
def _fixture_status_map():
    """Exercise coordinator transitions without claiming production mappings."""
    original = dict(parcels._STATUS_MAP)
    parcels._STATUS_MAP.update({
    ...
    yield
    parcels._STATUS_MAP.clear()
    parcels._STATUS_MAP.update(original)
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["events"][-1]["status"] = "IN_TRANSIT"
    return sample
...
async def test_update_merges_multiple_parcels(hass):
    ...
    coordinator = ELTACourierCoordinator(hass, client, entry)

    data = await coordinator._async_update_data()

    assert [parcel["barcode"] for parcel in data] == [ACTIVE_CODE]
    assert [parcel["barcode"] for parcel in coordinator.delivered] == [DELIVERED_CODE]
...
async def test_wrong_number_shape_lands_on_not_found_not_error(hass, caplog):
    """The real 'unknown code' payload (string result) is a quiet not-found,
    not a logged fetch failure — end to end through the real API client."""
    caplog.set_level(logging.WARNING)
    entry = _entry_with([{CONF_TRACKING_CODE: OTHER_CODE}])
    entry.add_to_hass(hass)

    response = AsyncMock(status=200, headers={})
    response.text = AsyncMock(return_value=json.dumps(not_found_envelope(OTHER_CODE)))
    context = MagicMock()
    ...
    coordinator = ELTACourierCoordinator(hass, ELTACourierApiClient(session), entry)
    data = await coordinator._async_update_data()

    assert len(data) == 1
    assert data[0]["barcode"] == OTHER_CODE
    assert data[0]["status"] == ParcelStatus.UNKNOWN
    assert coordinator.last_success_time is not None  # not counted as a failed fetch
    assert "ELTA Courier fetch failed" not in caplog.text
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = ELTACourierApiError("HTTP 500")
...
async def test_update_prunes_cache_for_untracked_parcels(hass):
    ...
    client.async_get_parcel.return_value = delivered_sample()
    coordinator = ELTACourierCoordinator(hass, client, entry)
    coordinator._raw_cache["GONE"] = {"tracking_code": "GONE", "events": []}
...
async def test_delivered_code_skipped_from_fetch(hass):
    ...
    # The delivered code is served from cache; only the active one is refetched.
...
async def test_update_fetches_parcels_sequentially(hass):
    """ELTA is rate-sensitive, so lookups are strictly one-by-one."""
    import asyncio
...
async def test_does_not_infer_delivery_time_changed_event(hass):
    entry = _entry_with([{CONF_TRACKING_CODE: ACTIVE_CODE}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = ELTACourierCoordinator(hass, client, entry)
