...
Home Assistant custom integration for **ELTA Courier** parcel tracking.
...
private `carrier-research/elta-courier/api/` and are **never** copied here.
...
| add/rename a parcel field, a `ParcelStatus`, or a bus event; change the sort/first-refresh; touch unmapped-status logging | *Parcel contract* — exact key set, units, sort, events + suppression; `test_parcels.py::test_normalize_keeps_only_confirmed_fields_and_raw_events` guards the key set |
...
- **If this carrier can reach `ParcelStatus.AT_PICKUP_POINT` from a real raw
  ...
  `ha-inpost` are reference implementations; `elta_courier` here does not
  demonstrate it yet.

## Carrier-specific notes

ELTA is an account-less, code-only integration. It sends exactly one form POST
per configured code and never fetches codes concurrently because the carrier is
rate-sensitive. The known payload does not yet prove timestamp timezone or any
status label's canonical meaning: all labels therefore remain `unknown` and
emit a one-shot WARNING with an issue link. Sender, receiver, ETA, pickup,
weight and dimensions are intentionally `None`; only the tracking-page URL and
optional event history are exposed. Diagnostics redact tracking codes and every
event `place` value.
...
refresh. `elta_courier/coordinator.py` is the canonical implementation
...
```
python -m pytest tests/ --cov=custom_components.elta_courier
```
