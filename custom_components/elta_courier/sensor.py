"""Sensor platform for the ELTA Courier parcel tracker integration."""
...
from . import ELTACourierConfigEntry
from .const import DOMAIN
from .coordinator import ELTACourierCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: ELTACourierConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up ELTA Courier sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        ELTACourierIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            ELTACourierParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(ELTACourierNextDeliverySensor(coordinator, entry))
    entities.append(ELTACourierDeliveredParcelsSensor(coordinator, entry))
    entities.append(ELTACourierLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class ELTACourierIncomingParcelsSensor(
    CoordinatorEntity[ELTACourierCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: ELTACourierCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                ELTACourierParcelSensor(self.coordinator, self._entry, barcode)
...
class ELTACourierParcelSensor(CoordinatorEntity[ELTACourierCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked ELTA Courier parcel."""
    ...
    def __init__(
        self, coordinator: ELTACourierCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class ELTACourierNextDeliverySensor(
    CoordinatorEntity[ELTACourierCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: ELTACourierCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class ELTACourierDeliveredParcelsSensor(
    CoordinatorEntity[ELTACourierCoordinator], SensorEntity
):
    """Recently delivered tracked ELTA Courier parcels."""
...
class ELTACourierLastUpdateSensor(
    CoordinatorEntity[ELTACourierCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when ELTA Courier was last polled successfully."""
