"""Calendar platform for the ELTA Courier parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the ELTA Courier deliveries calendar from a config entry."""
    async_add_entities([ELTACourierDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class ELTACourierDeliveriesCalendar(CoordinatorEntity[ELTACourierCoordinator], CalendarEntity):
    """A read-only calendar of expected ELTA Courier deliveries.
    ...
    def __init__(self, coordinator: ELTACourierCoordinator, entry: ConfigEntry) -> None:
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "ELTA Courier parcel")
            description_parts = [
