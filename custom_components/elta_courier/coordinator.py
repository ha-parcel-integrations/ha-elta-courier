"""Coordinator for the ELTA Courier parcel tracker integration.
...
from .api import ELTACourierApiClient, ELTACourierApiError
from .const import (
...
class ELTACourierCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        # ELTA is rate-sensitive: never issue concurrent lookups.
        results: list[dict | BaseException | None] = []
        for code in codes_to_fetch:
            try:
                results.append(await self._client.async_get_parcel(code))
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (ELTACourierApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, ELTACourierApiError)
                    and result.status_code == 429
                ):
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("ELTA Courier fetch failed for %s: %s", code, result)
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "ELTA Courier rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("ELTA Courier unreachable for all tracked parcels")
