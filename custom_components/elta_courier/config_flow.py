"""Config flow for the ELTA Courier parcel tracker integration."""
...
def normalize_tracking_code(value: str) -> str:
    """Preserve ELTA's unverified code format, trimming only outer whitespace."""
...
class ELTACourierConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the ELTA Courier integration."""
    ...
    ) -> ELTACourierOptionsFlowHandler:
        """Return the options flow handler."""
        return ELTACourierOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create the ELTA Courier hub — single instance, no input needed.
        ...
        the ``elta_courier.track_parcel`` service or a dashboard button.
...
class ELTACourierOptionsFlowHandler(OptionsFlow):
