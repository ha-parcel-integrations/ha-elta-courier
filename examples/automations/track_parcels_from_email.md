# Automatic parcel tracking from e-mail (IMAP → ELTA Courier)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the ELTA Courier tracking code, and register it with `elta_courier.track_parcel` — fully automatic, no extra custom component required.

ELTA Courier is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `elta_courier.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ elta_courier.track_parcel
                                     │
...
- This integration, with the `elta_courier.track_parcel` action available (field `tracking_code`).
...
ELTA has no confirmed public tracking-code format. The example deliberately
uses no regex: a broad pattern would collect unrelated order references. Use
the AI fallback for ELTA delivery emails, or add a narrow personal matcher only
after you have verified it against your own mail.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@elta_couriershipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: EXAMPLE123456","initial":true,"folder":"INBOX","username":"test"}'
```

Then `elta_courier.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
